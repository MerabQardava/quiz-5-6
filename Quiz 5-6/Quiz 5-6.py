...
def register():
    if request.method == 'POST':
        ...
        if  username=='' or password=='':
            ...
            b1 = auth(username=username, password=password,img='https://i1.wp.com/researchictafrica.net/wp/wp-content/uploads/2016/10/default-profile-pic.jpg?fit=300%2C300&ssl=1')
            db.session.add(b1)
            db.session.commit()
...
def user():
    if request.method == 'POST':
        ...
        db.session.commit()
        return render_template('user.html', img=auth.query.first().img)
    return render_template('user.html',  img=auth.query.first().img)
...
def userage(genrename):
    ...
    b = requests.get( f'https://api.nytimes.com/svc/books/v3//lists/current/{genrename}.json?&api-key=XsLIhP0Mg2Wr77ol3kIBaD81BdIsx9My')
    res2 = b.json()
    booklist=[]
